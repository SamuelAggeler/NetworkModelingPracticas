...
# Christoph Stadtfeld, Viviana Amati, Alvaro Uzaheta, Kieran Mepham 
# Social Network Lab
...
setwd("C:/Users/mephamk/Google Drive/Course Docs/PhD/ETH - Social Networks through Time/Teaching/network_modelling_2021") 
list.files()
...
# I will upload a file with more info on plotting in ggraph after class
...
theta1 <- model0$coef
p <- exp(theta1)/(1+exp(theta1)) # TODO: replace with the right value
p
...
pRecip <- exp(theta1+theta2)/(1+exp(theta1+theta2)) # TODO: replace this with the right value

# as mentioned in class, in these simple models (tie independence, dyadic dependence)
# you can calculate the probabilities to see the link with the estimates.
# Since this concerns the statistic of reciprocity, you can check the probabilites
# with the dyad census (table below has 202 empty dyads, 60 asymmetric, 38 reciprocal)

dyads<-table(friendship+t(friendship))/2

# the probability of a tie which would *not* reciprocate another tie is the 
# number of nonreciprocal ties, over all cases where the opportunity existed
# for a nonreciprocal tie; i.e. 60 ties which are in asymmetric dyads, 
# divided by the 60 opportunities for them in addition to 202*2 opportunities in empty dyads

60/(60+202*2)
pNorecip # nearly identical, but the estimator is not likely to give an exact match


# the probability of a tie which would reciprocate a tie is the number
# of times we observe a tie like this (i.e., 2*reciprocal dyads, since
# each dyad contains two ties), divided by the number of ties (since
# every tie gives the 'opportunity' for a reciprocal tie to form) 
...
model3 <- ergm(netw ~ edges + mutual + gwesp(decay=0.3,fixed=TRUE)
               ...
               + nodeifactor('delinquency')+ nodeofactor('delinquency'))
...
p1 <- ggraph(netw)+
  ...
  geom_node_point(size = 5, 
                  aes(shape = as.factor(gender), # 1 = girl, 2= boy
                      colour = delinquency) # delinquency level
  )+
  theme_graph()+
  ggtitle("Observed")
